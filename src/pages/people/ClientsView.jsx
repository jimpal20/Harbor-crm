...
const ClientsView = () => {
  ...
  const loadContacts = async () => {
    try {
      ...
      // Filter for Active and Prime status contacts
      const clientContacts = allContacts.filter(contact => 
        ['Active', 'Prime'].includes(contact.status)
      );
  ...
  const handleCreateContact = async (contactData) => {
    try {
      // Ensure status is either Active or Prime
      const validStatus = ['Active', 'Prime'].includes(contactData.status);
      if (!validStatus) {
      ...
      <div className="px-6 py-4 border-b">
        <div className="flex items-center space-x-4 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Active and Prime contacts</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {filteredContacts.map((contact) => (
              <div
                key={contact.id}
                className="bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-200"
              >
